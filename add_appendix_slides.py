#!/usr/bin/env python3
"""
Add 6 appendix slides (20-25) to the Attune Steel Investment Deck.
...
- The gap Attune fills: Voice is the only passive, hardware-free biomarker with direct neurological links to mental state
...
Every existing solution requires active effort — scheduling, showing up, typing, meditating. QS users don't want another chore; they want another data stream. Attune is the only passive mental health monitoring tool."""
...
ATTUNE'S 10 ENGAGEMENT FEATURES MAPPED TO DOPAMINE LOOPS (Section 2C):
...
Benchmarks: Typical SaaS 75-85% (Benchmarkit 2024), Top-quartile software 85%+, Oura ~60% (HW blend), Peloton ~40% (HW drag), Attune 82-92%.
...
Key insight: Software-only model = breakeven at MUCH lower subscriber count than hardware companies. Oura needed millions of ring sales; Attune needs ~2,000-4,000 subscribers.
...
- DERIVED: CAC per paid subscriber (CPI / conversion rate), Attune gross margin projections, breakeven subscriber counts
...
def slide_21_spending_failures(prs):
    ...
    spend_items = [
        ...
        {"text": "Attune\u2019s price fits the envelope", "size": 11, "color": BODY_GRAY},
    ]
...
def slide_22_behavioral_science(prs):
    ...
    add_text_box(slide, Inches(1.1), Inches(1.95), Inches(11.2), Inches(0.7),
                 ...
                 "Attune is a monitoring tool, not a treatment app.",
...
def slide_24_unit_economics(prs):
    ...
    margin_lines = [
        {"text": "Attune (App Store): 81.7%", "size": 13, "bold": True, "color": ACCENT_GREEN, "space_after": 4},
        {"text": "Attune (Direct): 91.8%", "size": 13, "bold": True, "color": ACCENT_GREEN, "space_after": 10},
