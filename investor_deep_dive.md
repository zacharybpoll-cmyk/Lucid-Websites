# Attune Steel — Investor Deep Dive
...
**The takeaway**: Every existing solution requires active effort — scheduling, showing up, typing, meditating. QS users don't want another chore; they want another data stream. Attune is the only passive mental health monitoring tool.
...
**The gap Attune fills**: Voice is the only passive, hardware-free biomarker with direct neurological links to mental state. No additional device. No new habit. Just speak for 25 seconds.
...
**The takeaway**: Our early adopter demographic — tech-forward, QS-oriented professionals — is also the demographic most affected by burnout and mental health challenges. They have the means, the motivation, and the existing data habits to adopt Attune.

---
---

# Section 2: Behavioral Science of Attune Steel

> **Goal**: Show that Attune's engagement architecture is deliberately designed using proven behavioral science — not accidental. Map each app feature to its psychological mechanism and explain why previous mental health solutions lacked these.
...
- **Key insight**: Attune is a **monitoring tool** (like Oura), not a **treatment app** (like Calm). This is the single most important architectural decision we made.

---

## 2B. Behavioral Science Frameworks Attune Draws On

| Framework | Key Principle | How Attune Uses It |
|-----------|--------------|-------------------|
...
## 2C. Attune's 10 Engagement Features Mapped to Dopamine Loops
...
**Key advantage**: On-device processing means zero compute COGS — the #1 variable cost for most AI/health SaaS. Attune's margin profile is closer to a pure software company than an AI company.
...
- **Attune: 82–92%** (software-only, on-device)
...
- **Key insight**: Software-only model means breakeven happens at a MUCH lower subscriber count than hardware companies. Oura needed millions of ring sales; Attune needs ~2,000–4,000 subscribers.
...
**Revenue multiple implications for Attune**:
...
| **DERIVED** (calculated from available data) | CAC per paid subscriber (CPI / conversion rate), Attune gross margin projections, breakeven subscriber counts |
