...
class SpeakerGate:
    def __init__(self, speaker_verifier, sample_rate: int = config.SAMPLE_RATE,
        ...
        self._verify_queue: deque = deque()  # segments waiting for verification
    ...
    def _verify_segment(self, segment_chunks):
        ...
        try:
            ...
            print(f"[SpeakerGate] Verification error: {e}, passing through")
            verified = True
            similarity = -1.0
        ...
        if verified and self._pending_reject is not None:
            ...
            if pending_sim >= config.SPEAKER_GATE_SANDWICH_THRESHOLD:
                ...
                print(f"[SpeakerGate] Sandwich-recovered pending segment ({pending_dur:.1f}s, similarity={pending_sim:.3f})")
        ...
        if verified:
            ...
            if self._consecutive_verified >= config.SPEAKER_GATE_MOMENTUM_WINDOW and not self._momentum_active:
                ...
                print(f"[SpeakerGate] Momentum activated (threshold → {config.SPEAKER_GATE_MOMENTUM_THRESHOLD})")
        else:
            self._consecutive_rejected += 1
            self._consecutive_verified = 0
            if self._consecutive_rejected >= config.SPEAKER_GATE_MOMENTUM_DECAY and self._momentum_active:
                self._momentum_active = False
                print(f"[SpeakerGate] Momentum deactivated (threshold → {config.SPEAKER_GATE_THRESHOLD})")
        ...
        if verified:
            ...
            print(f"[SpeakerGate] Verified segment ({dur:.1f}s, similarity={similarity:.3f}){momentum_tag}")
        else:
            self._stats['segments_rejected'] += 1
            self._last_verified = False
            print(f"[SpeakerGate] Rejected segment ({dur:.1f}s, similarity={similarity:.3f}){tag}")
    ...
    def flush_remaining(self):
        ...
        with self._lock:
            ...
            if remaining > 0:
                print(f"[SpeakerGate] Discarding {remaining:.1f}s remaining (too short to verify)")
            self._chunks.clear()
            self._total_samples = 0
