    ...
    def _fire_webhook(self, url: str, trigger_type: str, payload: Dict[str, Any]):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.debug(f"Webhook delivered to {url} (attempt {attempt + 1})")
                return
            except (urllib.error.URLError, Exception) as e:
                if attempt < max_attempts - 1:
                    delay = 2 ** attempt  # Exponential backoff: 1s, 2s
                    logger.warning(f"Webhook delivery to {url} failed (attempt {attempt + 1}/{max_attempts}), retrying in {delay}s: {e}")
                    time.sleep(delay)
                else:
                    logger.error(f"Webhook delivery to {url} failed after {max_attempts} attempts: {e}")
