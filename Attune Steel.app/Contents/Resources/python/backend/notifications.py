...
class NotificationManager:
    ...
    def __init__(self, db):
        ...
        # TS-008: Bounded deque for rate limit tracking (max 1 hour of sends).
        # MAX_PER_HOUR caps at 4, so maxlen=10 gives generous headroom.
        self._recent_sends: deque = deque(maxlen=10)
        self.MAX_PER_HOUR = 4
        # TS-003: Shutdown event for graceful timer cancellation
    ...
    def _can_send(self, notif_type: str) -> bool:
        ...
        # TS-008: _recent_sends is a deque; prune entries older than 1 hour
        with self._lock:
        ...
        with self._lock:
            if now >= target:
                ...
                if not self._curtain_sent_today:
                    try:
                        ...
                        logger.warning("Can't start curtain call thread (thread limit)")
            ...
            if delay > 0:
                ...
                try:
                    ...
                    logger.warning("Can't schedule curtain call timer (thread limit)")
    ...
    def _fire_curtain_call(self):
        ...
        # Schedule for tomorrow (never re-fire today to prevent thread bomb)
        self._schedule_tomorrow()

    def _schedule_tomorrow(self):
        """Schedule curtain call for tomorrow at 5:30pm. No immediate retry."""
        now = datetime.now()
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Can't schedule tomorrow's curtain call (thread limit)")
    ...
    def stop(self):
        ...
        TS-003: Signals shutdown and cancels curtain call timer."""
