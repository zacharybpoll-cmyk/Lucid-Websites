#!/usr/bin/env python3
"""
Attune Steel Investment Deck Generator — v3
...
IMG_DASHBOARD = os.path.join(BASE, "attune-steel-website/actual-websites/images/dashboard-health-score.png")
IMG_PUBMED = os.path.join(BASE, "attune-steel-website/actual-websites/images/pubmed-study.png")
IMG_LIFESTYLE = os.path.join(BASE, "attune-steel-website/actual-websites/images/lifestyle-attune-man.png")
IMG_ICON = os.path.join(BASE, "assets/icon.png")
OUTPUT = os.path.join(BASE, "Attune Steel Investment Deck.pptx")
...
def slide_01_title(prs):
    ...
    add_text_box(slide, Inches(2.5), Inches(3.5), Inches(8.3), Inches(0.9),
                 "Attune.", font_size=54, color=WHITE, bold=True,
...
def slide_03_qs_market(prs):
    ...
    add_text_box(slide, Inches(0.8), Inches(1.85), Inches(11.5), Inches(0.5),
                 "150-200M people are already paying for health data. Attune adds the missing metric.",
    ...
    # Attune positioning card
    attune_y = Inches(5.2)
    attune_box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE,
                                         right_x, attune_y, Inches(5.5), Inches(0.8))
    attune_box.fill.solid()
    attune_box.fill.fore_color.rgb = NAVY
    attune_box.line.fill.background()
    attune_box.adjustments[0] = 0.08

    add_text_box(slide, right_x + Inches(0.3), attune_y + Inches(0.1), Inches(2.5), Inches(0.35),
                 "Attune", font_size=18, color=WHITE, bold=True, font_name=HEADLINE_FONT)
    add_text_box(slide, right_x + Inches(3.0), attune_y + Inches(0.1), Inches(2.2), Inches(0.35),
                 "$14.99/mo", font_size=18, color=STEEL, bold=True,
                 alignment=PP_ALIGN.RIGHT, font_name=HEADLINE_FONT)
    add_text_box(slide, right_x + Inches(0.3), attune_y + Inches(0.45), Inches(4.8), Inches(0.3),
    ...
    add_text_box(slide, Inches(0.8), Inches(6.45), Inches(11.5), Inches(0.4),
                 "Zero voice-first mental health players in this ecosystem. Attune is the first.",
...
def slide_09_technology(prs):
    ...
    add_text_box(slide, Inches(0.8), Inches(5.7), Inches(11.5), Inches(0.8),
                 ...
                 "Attune inherits validated, peer-reviewed voice biomarker AI without the regulatory trap.",
...
def slide_11_market(prs):
    ...
    add_divider_line(slide, Inches(0.8), callout_y, Inches(11.7), RGBColor(0xE8, 0xE8, 0xF0), Pt(1))
    add_text_box(slide, Inches(0.8), callout_y + Inches(0.15), Inches(11.5), Inches(0.5),
                 "Attune is the only voice-first, clinically validated player targeting the consumer QS segment.",
                 font_size=16, color=CHARCOAL, bold=True)
...
def slide_12_business_model(prs):
    ...
    add_text_box(slide, price_x + Inches(0.4), price_y + Inches(0.3),
                 ...
                 "Attune Premium", font_size=14, color=STEEL, bold=True)
...
def slide_13_competitive_landscape(prs):
    ...
    # Competitor dots — UPDATED: bigger Attune, dead competitor labels, Woebot shut down
    competitors = [
    ...
    # ATTUNE — much bigger dot with highlight ring
    attune_cx = matrix_x + Inches(5.0)
    attune_cy = matrix_y + Inches(0.3)
    attune_dot_size = Inches(0.4)

    # Highlight ring (glow effect)
    ring_size = Inches(0.6)
    ring = slide.shapes.add_shape(MSO_SHAPE.OVAL,
                                   attune_cx - Inches(0.1), attune_cy - Inches(0.1),
                                   ring_size, ring_size)
    ring.fill.solid()
    ...
    # Attune dot
    attune_dot = slide.shapes.add_shape(MSO_SHAPE.OVAL, attune_cx, attune_cy,
                                         attune_dot_size, attune_dot_size)
    attune_dot.fill.solid()
    attune_dot.fill.fore_color.rgb = STEEL
    attune_dot.line.fill.background()

    add_text_box(slide, attune_cx + attune_dot_size + Inches(0.1), attune_cy - Inches(0.02),
                 Inches(1.5), Inches(0.35),
                 "Attune", font_size=14, color=CHARCOAL, bold=True)
...
def slide_14_competitive_moats(prs):
    ...
    moats = [
        ...
        ("5", "Consumer Distribution", "Kintsugi and Woebot are dead. Sonde is B2B-only. Attune owns the consumer relationship. Direct-to-consumer = brand loyalty + higher LTV."),
    ]
...
def slide_15_comparable_valuations(prs):
    ...
    add_text_box(slide, Inches(0.8), Inches(1.1), Inches(11.5), Inches(0.8),
                 "Where Attune Fits in the Landscape",
                 font_size=32, color=CHARCOAL, bold=True, font_name=HEADLINE_FONT)
    ...
    add_text_box(slide, Inches(0.8), Inches(6.45), Inches(11.5), Inches(0.5),
                 "Attune = clinical rigor of Kintsugi (RIP) + consumer accessibility of Oura",
                 font_size=16, color=CHARCOAL, bold=True)
...
def slide_17_growth_projections(prs):
    ...
    add_text_box(slide, left_x, timeline_y, Inches(5.5), Inches(0.35),
                 "Attune Growth Trajectory", font_size=18, color=CHARCOAL,
                 bold=True, font_name=HEADLINE_FONT)
